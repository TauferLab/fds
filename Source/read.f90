...
INTEGER :: QUOTIENT,REMAINDER,BIG_BLOCK
...
! Compute variable values for block assignment of meshes to mpi processes

QUOTIENT = NMESHES / N_MPI_PROCESSES
REMAINDER = MODULO(NMESHES, N_MPI_PROCESSES)
BIG_BLOCK = REMAINDER*(QUOTIENT + 1)
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
	       ...
	       IF (NM<=BIG_BLOCK) THEN
                    CURRENT_MPI_PROCESS = ((NM - 1)/(QUOTIENT + 1))
               ELSE
                    CURRENT_MPI_PROCESS = REMAINDER + ((NM - BIG_BLOCK - 1)/QUOTIENT)
               ENDIF
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE,ALLOW_SURFACE_PARTICLES, &
                ...
                HUMIDITY,HVAC_LOCAL_PRESSURE,HVAC_MASS_TRANSPORT,HVAC_PRES_RELAX,IBLANK_SMV,LEVEL_SET_COUPLED,LEVEL_SET_ELLIPSE,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (VEG_LSET_FUEL_INDEX>0 .AND. LEVEL_SET_COUPLED) HRRPUA = 1._EB  ! Placeholder for future setting
